...
| `hours` | int | 否 | 时间窗口（小时），只转存该时间范围内的文件，默认 48。可在 `shareUrls` 内单项覆盖（统一使用 `??` 合并，写 `0` 即显式覆盖） |
...
| 页面 | 能力 |
| --- | --- |
| **配置** | 表单化编辑全部配置项，按「账号与目标 / 同步与下载 / 服务与调度」三个可折叠分组组织；`shareUrls` 用表格维护（一行一个分享，支持备用链接组与 per-share 覆盖项）；保存后**定时任务自动重载，无需重启** |
