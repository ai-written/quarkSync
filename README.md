...
| **日志** | 默认只看**汇总**（总的转存结果汇总、下载与清理结果、全部报错），另有「分享明细」（每个分享的筛选漏斗）、「全部日志」、「登录日志」三个视图；按级别与关键字过滤，可调行数、可自动刷新；日志很大也不卡 |
...
- **日志**写在项目根目录的 `sync.log`，按时间顺序追加，自动保留最近 7 天；网页默认只显示总结果的「汇总」，分享明细、全部日志与登录记录各有独立视图
