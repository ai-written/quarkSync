...
![网页管理界面 · 分享链接页](image.png)

*「分享链接」页（默认页；图中为示例数据）：一行一个分享，链接格写多行即为「备用链接组」（图中第 3 行标有「备用组」），每项可单独设置影视名称并覆盖多久内更新 / 最小体积 / 最多转存；底部保存栏在任何页面都可用。*

## 功能

- **自动转存** — 监控多个夸克网盘分享链接，将最近更新的文件自动转存到自己的网盘
- **网页管理界面** — 浏览器里维护配置、查看日志、手动触发任务，改完即时生效无需重启
- **过滤去重** — 按更新时间过滤，按"文件名+大小"自动去重，避免重复转存
- **小文件过滤** — 支持按文件大小下限过滤（`minFileSizeMB`），小于指定 MB 的文件不转存，可按分享链接单独配置
- **最多转存** — 支持每个分享链接最多转存 N 条最新文件（`maxFilesPerShare`），可按分享链接单独配置
...
| `password` | string | 否 | 默认提取码。网页界面**不再暴露**这个全局字段（提取码在「分享链接」页逐分享维护）；磁盘上已有的值仍会作为该分享未填时的回退生效，保存时原样保留 |
| `tip` | string | 否 | 默认文件名前缀。网页界面**不再暴露**这个全局字段（前缀改为在「分享链接」页逐个分享填写）；磁盘上已有的值仍会作为该分享未填时的回退生效，且保存配置时会原样保留 |
| `hours` | int/string | 否 | 只转存最近这段时间内更新过的文件（网页界面上叫「多久内更新」），默认 48 小时。支持单位写法：`30m` 分钟、`12h` 小时、`1d` 天、`1w` 周、`1mo` 月（30 天）、`1y` 年（365 天），也支持小数（`1.5d`）与纯数字（按小时）。可在 `shareUrls` 内单项覆盖 |
| `days` | int | 否 | 「多久内更新」的天数写法（历史字段），等价于 `hours = days * 24`；仅当未设置 `hours` 时生效 |
| `minFileSizeMB` | int | 否 | 最小文件体积过滤（MB，网页界面上叫「最小体积」），小于此值的文件不转存，默认 0 不过滤。可在 `shareUrls` 内单项覆盖 |
| `maxFilesPerShare` | int | 否 | 每个分享最多转存文件数（网页界面上叫「最多转存」），按更新时间倒序取最新 N 条，默认 0 不限制。可在 `shareUrls` 内单项覆盖 |
| `targetDirName` | string | 否 | 转存目标文件夹名称，默认 `来自：分享`。支持多级路径（如 `转存/来自：分享`）：按 `/` 拆分后逐级查找，不存在的层级会在上一级下自动创建。只支持从根目录往下的相对层级，`.` 与 `..` 无特殊含义，空段（开头 / 结尾 / 重复的 `/`）会被忽略 |
| `targetDirFid` | string | 否 | 转存目标文件夹 ID，优先级高于 `targetDirName`。网页界面**不再暴露**该字段（属高级选项，需直接编辑 `config.json`）；磁盘上已有的值仍会照常生效，保存配置时原样保留。因为它的优先级更高，只要磁盘上设了它，「配置」页就会给出提示，避免改了文件夹名却看不出效果 |
...
| `alistPath` | string | 否 | 从 AList 下载的**源目录**（AList 服务器上的路径，如 `/kuake/来自：分享`），其中的文件会下载到本地的 `downloadDir` |
...
> `hours`、`minFileSizeMB`、`maxFilesPerShare` 三者统一使用 `??`（nullish）合并，因此写 `0` 均为**显式覆盖**而非「未配置」。注意 `"hours": 0` 会把「多久内更新」设为 0 小时（即只转存"此刻及之后更新"的文件，实际等于不转存任何文件），通常不是你想要的结果；若只想沿用全局设置，请直接省略该字段。
...
| 页面 | 能力 |
| --- | --- |
| **分享链接**（默认页） | 表格化维护 `shareUrls`：一行一个分享，链接格写多行即为「备用链接组」（按序 failover），每项可单独设置影视名称（文件名前缀）与提取码，并覆盖多久内更新 / 最小体积 / 最多转存。底部常驻保存栏在任何页面都可用 |
| **配置** | 表单化编辑其余配置项，按「账号与目标 / 同步与下载 / 服务与调度」三个可折叠分组组织；保存后**定时任务自动重载，无需重启**。全局 `tip`、`password`、`targetDirFid` 有意不在界面出现（前缀与提取码逐分享维护，文件夹 ID 属高级选项）；磁盘上已有这些字段则保存时原样带过、不会被清掉 |
